...
function getNomorSuratBerikutnya($conn, $jenis_surat = 'SKU') {
    $tahun = date('Y');
...
if (isset($_POST['action']) && $_POST['action'] == 'simpan_surat') {
    ...
    $alamat_usaha = mysqli_real_escape_string($conn, $_POST['alamat_usaha'] ?? '');
    ...
    if ($penduduk) {
        ...
        $keterangan = "Jenis Usaha: $jenis_usaha\nNama Usaha: $nama_usaha\nAlamat Usaha: $alamat_usaha";
    ...
    header("Location: usaha.php?nik=" . urlencode($nik));
    exit();
}
...
if (isset($_POST['action']) && $_POST['action'] == 'download_pdf') {
    ...
    $dompdf->render();
    $dompdf->stream("surat_usaha_" . date('Ymd_His') . ".pdf", array("Attachment" => true));
    exit;
}
...
if (isset($_POST['action']) && $_POST['action'] == 'download_word') {
    ...
    // Kop Surat (Hanya dirender jika BUKAN mode blanko)
    if ($is_blanko === 'false') {
    ...
    $section->addText('SURAT KETERANGAN MEMILIKI USAHA', ['bold' => true, 'underline' => 'single', 'size'=>13], ['alignment' => Jc::CENTER]);
    ...
    $section->addTextBreak(0.5);
    $jenis_usaha_word = $_POST['jenis_usaha'] ?? 'Perdagangan';
    $nama_usaha_word = $_POST['nama_usaha'] ?? 'Toko';
    $alamat_usaha_word = $_POST['alamat_usaha'] ?? 'Dsn. Pangloros Desa Sukolilo Timur';
    
    $section->addText('Menerangkan dengan sebenarnya bahwa orang tersebut benar – benar penduduk Desa Sukolilo Timur Kecamatan Labang Kabupaten Bangkalan, dan bersangkutan benar – benar memiliki Usaha ' . $jenis_usaha_word . ' dengan nama ' . $nama_usaha_word . ' yang beralamat di ' . $alamat_usaha_word . '.');
    $section->addTextBreak(0.5);
    ...
    $filename = "surat_usaha_" . date('Ymd_His') . ".docx";
...
// ==================== AMBIL DATA PENDUDUK ====================
$penduduk = null;
$nomor_surat_otomatis = getNomorSuratBerikutnya($conn, 'SKU');
...
$pageTitle = "Surat Keterangan Usaha";
...
.card { border: none; border-radius: 1rem; box-shadow: 0 0.5rem 1rem rgba(0, 0, 0, 0.05); overflow: visible; /* Penting untuk dropdown */ }
...
.data-card.usaha-form { border-left: 5px solid #1cc88a; } /* Hijau untuk form usaha */
...
/* Form Elements Detail Usaha */
.usaha-form .form-label { color: #5a5c69; }
.usaha-form .form-control, .usaha-form .form-select { border-radius: 0.5rem; border: 1px solid #d1d3e2; padding: 0.6rem 1rem; font-size: 0.95rem; }
.usaha-form .form-control:focus, .usaha-form .form-select:focus { border-color: #1cc88a; box-shadow: 0 0 0 0.2rem rgba(28, 200, 138, 0.25); }
...
<div class="card mb-4" style="z-index: 999 !important;">
    <div class="card-header bg-primary text-white py-3">
        <h6 class="m-0 font-weight-bold"><i class="fas fa-search me-2"></i>Pencarian Data Pemohon Usaha</h6>
    </div>
...
<div class="data-card usaha-form">
    <h5 class="font-weight-bold text-success border-bottom pb-3 mb-4"><i class="fas fa-store me-2"></i>Rincian Usaha Pemohon</h5>
    <div class="row">
        <div class="col-md-4 mb-3">
            <label class="form-label fw-bold">Kategori Bisnis</label>
            <div class="input-group">
                <span class="input-group-text bg-white"><i class="fas fa-tags text-success"></i></span>
                <input type="text" class="form-control border-start-0" id="jenis_usaha" list="kategoriList" placeholder="Pilih atau ketik kategori..." value="Perdagangan">
                <datalist id="kategoriList">
                    <option value="Perdagangan"></option>
                    <option value="Pertanian"></option>
                    <option value="Perikanan"></option>
                    <option value="Peternakan"></option>
                    <option value="Industri Rumah Tangga"></option>
                    <option value="Jasa"></option>
                    <option value="Kerajinan"></option>
                    <option value="Kuliner"></option>
                    <option value="Toko Sembako"></option>
                    <option value="Lainnya"></option>
                </datalist>
            </div>
            <small class="text-muted" style="font-size: 0.75rem;">Bisa dipilih atau diketik sendiri</small>
        </div>
        <div class="col-md-4 mb-3">
            <label class="form-label fw-bold">Nama Toko/Usaha</label>
            <div class="input-group">
                <span class="input-group-text bg-white"><i class="fas fa-signature text-success"></i></span>
                <input type="text" class="form-control border-start-0" id="nama_usaha" placeholder="Contoh: Toko Berkah" value="Toko Sembako Berkah">
            </div>
        </div>
        <div class="col-md-4 mb-3">
            <label class="form-label fw-bold">Lokasi Usaha</label>
            <div class="input-group">
                <span class="input-group-text bg-white"><i class="fas fa-map-marked-alt text-success"></i></span>
                <input type="text" class="form-control border-start-0" id="alamat_usaha" placeholder="Alamat lengkap" value="Dsn. Pangloros Desa Sukolilo Timur">
            </div>
        </div>
    </div>
</div>
...
<form id="formBuatSurat" method="POST">
    ...
    <input type="hidden" name="jenis_usaha" id="hidden_jenis_usaha">
    <input type="hidden" name="nama_usaha" id="hidden_nama_usaha">
    <input type="hidden" name="alamat_usaha" id="hidden_alamat_usaha">
        ...
        <div class="editor-workspace">
            <div class="editor-container paper-a4" id="editor" contenteditable="true" style="padding: 2cm 2cm 2cm 3cm; font-family: 'Times New Roman', Times, serif; font-size: 12pt;">
                ...
                <div style="text-align: center; margin: 15px 0 20px; color: #000;">
                    <h4 style="text-decoration: underline; font-weight: bold; margin:0 0 3px 0; font-size: 13pt; color: #000;">SURAT KETERANGAN MEMILIKI USAHA</h4>
                ...
                <div style="color: #000;">
                    ...
                    <p style="margin-bottom: 10px; line-height: 1.6;">Menerangkan dengan sebenarnya bahwa orang tersebut benar – benar penduduk Desa Sukolilo Timur Kecamatan Labang Kabupaten Bangkalan, dan bersangkutan benar – benar memiliki Usaha <span id="jenis_usaha_text" contenteditable="true" style="font-weight: bold;">Perdagangan</span> dengan nama <span id="nama_usaha_text" contenteditable="true" style="font-weight: bold;">Toko Sembako Berkah</span> yang beralamat di <span id="alamat_usaha_text" contenteditable="true" style="font-weight: bold;" class="text-uppercase">Dsn. Pangloros Desa Sukolilo Timur</span>.</p>
                ...
                <table style="width: 100%; border: none; margin-top: 30px; color: #000; font-size: inherit;">
                    <tr>
                        ...
                        <td style="width: 45%; border: none; text-align: center; color: #000; vertical-align: top;">
                            ...
                            <span style="text-decoration: underline; font-weight: bold; color: #000;" contenteditable="true">H. ZAINAL ABIDIN</span>
                        </td>
                    </tr>
                </table>
                ...
                <div class="col-lg-5 mb-4 mb-lg-0 border-end pe-4">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control" id="keperluan" name="keperluan" placeholder="Misal: Perizinan Toko">
                        </div>
                    </div>
                </div>
                    ...
                    <table class="action-table">
                        ...
                        <tr>
                            <td colspan="2" style="padding-top: 5px;">
                                <a href="usaha.php" class="btn-action btn-preview" style="background: #fdfdfd;">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0">
            ...
            <div class="modal-body text-center py-5">
                ...
                <p class="text-muted mb-4">Pastikan data usaha dan data warga sudah benar sebelum menyimpan.</p>
...
// ===== EVENT LISTENER UNTUK SINKRONISASI FORM USAHA KE EDITOR =====
function initUsahaListeners() {
    ...
    if (jenisUsaha) {
        jenisUsaha.addEventListener('change', updateJenisUsaha);
        jenisUsaha.addEventListener('input', updateJenisUsaha);
    }
    if (namaUsaha) {
        namaUsaha.addEventListener('input', updateNamaUsaha);
    }
    if (alamatUsaha) {
        alamatUsaha.addEventListener('input', updateAlamatUsaha);
    }
}

function updateJenisUsaha() {
    const jenis = document.getElementById('jenis_usaha').value;
    const jenisText = document.getElementById('jenis_usaha_text');
    if (jenisText && jenis) jenisText.textContent = jenis;
}

function updateNamaUsaha() {
    const nama = document.getElementById('nama_usaha').value;
    const namaText = document.getElementById('nama_usaha_text');
    if (namaText && nama) namaText.textContent = nama;
}

function updateAlamatUsaha() {
    const alamat = document.getElementById('alamat_usaha').value;
    const alamatText = document.getElementById('alamat_usaha_text');
    if (alamatText && alamat) alamatText.textContent = alamat.toUpperCase();
}
...
function updateHiddenInputs() {
    ...
    if(jenisUsaha) document.getElementById('hidden_jenis_usaha').value = jenisUsaha.value;
    if(namaUsaha) document.getElementById('hidden_nama_usaha').value = namaUsaha.value;
    if(alamatUsaha) document.getElementById('hidden_alamat_usaha').value = alamatUsaha.value;
}
...
if (searchInput) {
    searchInput.addEventListener('input', function() {
        ...
        searchTimeout = setTimeout(() => {
            fetch('usaha.php?search_penduduk=' + encodeURIComponent(keyword))
...
function submitFormWithAction(action, blank = false) {
    updateHiddenInputs(); // Harus dipanggil agar input usaha terkirim
