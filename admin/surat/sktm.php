...
function getNomorSuratBerikutnya($conn, $jenis_surat = 'SKTM') {
    $tahun = date('Y');
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (count($parts) >= 3) {
            ...
            // Format disesuaikan dengan standar SKTM, ubah jika ada standar lain
            return $nomor . ' / 422 / 433.311.5 / ' . $tahun;
        }
    }
    return '1 / 422 / 433.311.5 / ' . $tahun;
}
...
if (isset($_POST['action']) && $_POST['action'] == 'simpan_surat') {
    ...
    if ($penduduk) {
        ...
        $keterangan = "Tujuan SKTM: $keperluan";
    ...
    header("Location: sktm.php?nik=" . urlencode($nik));
    exit();
}
...
if (isset($_POST['action']) && $_POST['action'] == 'download_pdf') {
    ...
    $dompdf->render();
    $dompdf->stream("surat_sktm_" . date('Ymd_His') . ".pdf", array("Attachment" => true));
    exit;
}
...
if (isset($_POST['action']) && $_POST['action'] == 'download_word') {
    ...
    $section->addText('SURAT KETERANGAN TIDAK MAMPU', ['bold' => true, 'underline' => 'single', 'size'=>13], ['alignment' => Jc::CENTER]);
    ...
    $section->addTextBreak(0.5);
    $keperluan_word = $_POST['keperluan'] ?? 'mendapatkan bantuan berupa rehab/perbaikan rumah tempat tinggal';
    
    // Paragraf disesuaikan dengan referensi gambar SKTM
    $paragrafText = 'Bahwa nama yang tercantum di atas adalah benar-benar berdomisili di Desa Sukolilo Timur, Kecamatan Labang. Sepanjang pengamatan kami dan sesuai data yang ada dalam catatan kependudukan, orang tersebut di atas benar tergolong dalam keluarga prasejahtera (Keluarga Berpenghasilan Rendah). Surat Keterangan ini diberikan untuk ' . $keperluan_word . '.';
    
    $section->addText($paragrafText, [], ['alignment' => Jc::BOTH]);
    $section->addTextBreak(0.5);
    $section->addText('Demikian surat keterangan ini dibuat dengan sebenarnya dan diberikan kepada yang bersangkutan untuk dapat dipergunakan sebagaimana mestinya.', [], ['alignment' => Jc::BOTH]);
    ...
    $filename = "surat_sktm_" . date('Ymd_His') . ".docx";
...
// ==================== AMBIL DATA PENDUDUK ====================
$penduduk = null;
$nomor_surat_otomatis = getNomorSuratBerikutnya($conn, 'SKTM');
...
$pageTitle = "Surat Keterangan Tidak Mampu (SKTM)";
...
.data-card.keperluan-form { border-left: 5px solid #e74a3b; } /* Merah/Penting untuk SKTM */
...
/* Form Elements Detail Keperluan */
.keperluan-form .form-label { color: #5a5c69; }
.keperluan-form .form-control { border-radius: 0.5rem; border: 1px solid #d1d3e2; padding: 0.8rem 1rem; font-size: 1rem; resize: none; }
.keperluan-form .form-control:focus { border-color: #e74a3b; box-shadow: 0 0 0 0.2rem rgba(231, 74, 59, 0.25); }
...
<div class="data-card keperluan-form">
    <h5 class="font-weight-bold text-danger border-bottom pb-3 mb-4"><i class="fas fa-hand-holding-heart me-2"></i>Maksud dan Tujuan SKTM</h5>
    <div class="row">
        <div class="col-md-12 mb-2">
            <label class="form-label fw-bold">Tuliskan tujuan permohonan SKTM ini</label>
            <div class="input-group shadow-sm">
                <span class="input-group-text bg-white"><i class="fas fa-pen-fancy text-danger"></i></span>
                <textarea class="form-control border-start-0" id="keperluan_text" rows="2" placeholder="Contoh: Mendapatkan bantuan rehab/perbaikan rumah, Persyaratan Beasiswa, dll">mendapatkan bantuan berupa rehab/perbaikan rumah tempat tinggal</textarea>
            </div>
            <small class="text-muted mt-2 d-inline-block"><i class="fas fa-info-circle me-1"></i> Teks yang diketik akan otomatis muncul di dalam paragraf inti surat SKTM di bawah.</small>
        </div>
    </div>
</div>
        ...
        <div class="editor-workspace">
            <div class="editor-container paper-a4" id="editor" contenteditable="true" style="padding: 2cm 2cm 2cm 3cm; font-family: 'Times New Roman', Times, serif; font-size: 12pt;">
                ...
                <div style="text-align: center; margin: 15px 0 20px; color: #000;">
                    <h4 style="text-decoration: underline; font-weight: bold; margin:0 0 3px 0; font-size: 13pt; color: #000;">SURAT KETERANGAN TIDAK MAMPU</h4>
                ...
                <div style="color: #000;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px; color: #000; font-size: inherit;">
                        <tr>
                    ...
                    <p style="margin-bottom: 10px; line-height: 1.6; text-align: justify;">Bahwa nama yang tercantum di atas adalah benar-benar berdomisili di <span contenteditable="true" class="font-weight-bold text-uppercase">Desa Sukolilo Timur, Kecamatan Labang</span>. Sepanjang pengamatan kami dan sesuai data yang ada dalam catatan kependudukan, orang tersebut di atas benar tergolong dalam keluarga prasejahtera (Keluarga Berpenghasilan Rendah). Surat Keterangan ini diberikan untuk <span id="keperluan_surat" contenteditable="true" style="font-weight: bold;">mendapatkan bantuan berupa rehab/perbaikan rumah tempat tinggal</span>.</p>
        ...
        <div class="card-footer bg-white py-4 border-top-0 rounded-bottom">
            <div class="row align-items-center">
                <div class="col-lg-5 mb-4 mb-lg-0 border-end pe-4">
                    <h6 class="font-weight-bold text-primary mb-3"><i class="fas fa-cog me-2"></i>Pengaturan Arsip</h6>
                    <div class="row">
                        <div class="col-12">
                            <label class="form-label small font-weight-bold text-gray-600 mb-1">Nomor Surat Otomatis</label>
                    ...
                    <table class="action-table">
                        ...
                        <tr>
                            <td colspan="2" style="padding-top: 5px;">
                                <a href="sktm.php" class="btn-action btn-preview" style="background: #fdfdfd;">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0">
            ...
            <div class="modal-body text-center py-5">
                ...
                <p class="text-muted mb-4">Pastikan data penduduk dan tujuan SKTM sudah benar sebelum menyimpan.</p>
...
// ===== EVENT LISTENER UNTUK SINKRONISASI FORM KEPERLUAN KE EDITOR =====
function initKeperluanListeners() {
    const keperluanInput = document.getElementById('keperluan_text');
    const keperluanSpan = document.getElementById('keperluan_surat');
    
    if (keperluanInput && keperluanSpan) {
        keperluanInput.addEventListener('input', function() {
            keperluanSpan.textContent = this.value;
        });
    }
}

function updateHiddenInputs() {
    const keperluan = document.getElementById('keperluan_text').value;
    document.getElementById('hidden_keperluan').value = keperluan;
}
...
if (searchInput) {
    searchInput.addEventListener('input', function() {
        ...
        searchTimeout = setTimeout(() => {
            fetch('sktm.php?search_penduduk=' + encodeURIComponent(keyword))
...
function submitFormWithAction(action, blank = false) {
    updateHiddenInputs(); // Harus dipanggil agar input keperluan terkirim
