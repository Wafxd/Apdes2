...
if (isset($_POST['action']) && $_POST['action'] == 'simpan_surat') {
    ...
    if ($penduduk) {
        ...
        $query_insert = "INSERT INTO arsip_surat 
                        ...
                        VALUES 
                        ('$no_surat', '$jenis_surat', '$tanggal_surat', '$nik', '$nama_pemohon',
                         '{$penduduk['tempat_lahir']}', '{$penduduk['tanggal_lahir']}', '{$penduduk['alamat']}', '$keperluan', '$keterangan', '{$_SESSION['id_admin']}')";
...
if (isset($_POST['action']) && $_POST['action'] == 'download_word') {
    ...
    $section->addText('SURAT KETERANGAN KEHILANGAN', ['bold' => true, 'underline' => 'single', 'size'=>13], ['alignment' => Jc::CENTER]);
    ...
    $dataRows = [
        ['N a m a', strtoupper($penduduk_word['nama_penduduk'] ?? '')],
        ...
        ['Alamat', strtoupper($penduduk_word['alamat'] ?? '')]
    ];
    ...
    foreach ($dataRows as $row) {
        ...
        $tableData->addCell(7000)->addText($row[1], ['bold' => ($row[0] === 'N a m a' || $row[0] === 'Alamat') ? true : false]);
    }
    
    $section->addTextBreak(0.5);
    $hari_k = $_POST['hari_kejadian'] ?? '';
    $tgl_k = $_POST['tanggal_kejadian'] ?? '';
    if (!empty($tgl_k)) $tgl_k = date('d-m-Y', strtotime($tgl_k));
    $jam_k = $_POST['jam_kejadian'] ?? '';
    
    $section->addText('menerangkan dengan sebenarnya bahwa orang tersebut diatas pada hari ' . $hari_k . ' tanggal ' . $tgl_k . ' sekitar pukul ' . $jam_k . ' telah kehilangan :');
...
.data-card.kejadian-form { border-left: 5px solid #f6c23e; } /* Kuning untuk form kehilangan/kejadian */
...
/* Form Elements Detail Kejadian */
.kejadian-form .form-label { color: #5a5c69; }
.kejadian-form .form-control { border-radius: 0.5rem; border: 1px solid #d1d3e2; padding: 0.6rem 1rem; font-size: 0.95rem; }
.kejadian-form .form-control:focus { border-color: #f6c23e; box-shadow: 0 0 0 0.2rem rgba(246, 194, 62, 0.25); }
.dokumen-item { display: flex; gap: 10px; margin-bottom: 12px; align-items: center; }
.dokumen-item input { flex: 1; border-radius: 0.5rem; border: 1px solid #d1d3e2; padding: 0.5rem 1rem; }
.btn-add-dokumen { background: #1cc88a; color: white; border: none; border-radius: 0.5rem; padding: 8px 15px; cursor: pointer; font-size: 0.9rem; font-weight: 600; transition: all 0.2s; }
.btn-add-dokumen:hover { background: #13855c; transform: translateY(-1px); }
.btn-remove-dokumen { background: #e74a3b; color: white; border: none; border-radius: 0.5rem; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; }
.btn-remove-dokumen:hover { background: #be2617; }
...
<div class="card mb-4" style="z-index: 999 !important;">
    <div class="card-header bg-primary text-white py-3">
        <h6 class="m-0 font-weight-bold"><i class="fas fa-search me-2"></i>Pencarian Data Pemohon</h6>
    </div>
...
<div class="data-card">
    ...
    <div class="row text-gray-800">
        <div class="col-md-6">
            ...
            <div class="mb-3"><span class="data-label"><i class="fas fa-user me-2 text-gray-400"></i>Nama</span> <span class="me-2">:</span> <span class="data-value fs-6 font-weight-bold text-uppercase"><?php echo $penduduk['nama_penduduk']; ?></span></div>
        ...
        <div class="col-md-6">
            ...
            <div class="mb-3"><span class="data-label"><i class="fas fa-home me-2 text-gray-400"></i>Alamat</span> <span class="me-2">:</span> <span class="data-value text-uppercase"><?php echo $penduduk['alamat']; ?></span></div>
        </div>
    </div>
</div>

<div class="data-card kejadian-form">
    <h5 class="font-weight-bold text-warning border-bottom pb-3 mb-4"><i class="fas fa-exclamation-triangle me-2"></i>Rincian Kejadian Kehilangan</h5>
    <div class="row">
        <div class="col-md-4 mb-3">
            <label class="form-label fw-bold">Tanggal Kejadian</label>
            <div class="input-group">
                <span class="input-group-text bg-white"><i class="fas fa-calendar-day text-warning"></i></span>
                <input type="date" class="form-control border-start-0" id="tanggal_kejadian" value="<?php echo date('Y-m-d'); ?>">
            </div>
        </div>
        <div class="col-md-4 mb-3">
            <label class="form-label fw-bold">Waktu/Jam Perkiraan</label>
            <div class="input-group">
                <span class="input-group-text bg-white"><i class="fas fa-clock text-warning"></i></span>
                <input type="time" class="form-control border-start-0" id="jam_kejadian" value="<?php echo date('H:i'); ?>">
            </div>
        </div>
        <div class="col-md-4 mb-3">
            <label class="form-label fw-bold">Hari Kejadian</label>
            <div class="input-group">
                <span class="input-group-text bg-white"><i class="fas fa-calendar-week text-warning"></i></span>
                <input type="text" class="form-control border-start-0 bg-light" id="hari_kejadian" value="<?php echo hari_indonesia(date('Y-m-d')); ?>" readonly>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 mb-4">
            <label class="form-label fw-bold">Lokasi/Tempat Kejadian</label>
            <div class="input-group">
                <span class="input-group-text bg-white"><i class="fas fa-map-marker-alt text-warning"></i></span>
                <input type="text" class="form-control border-start-0" id="lokasi_kejadian" placeholder="Deskripsikan tempat kejadian (misal: di sekitar Pasar Baru)" value="Diantara Jalan Sukolilo Sampai Jalan Pasar Labang">
            </div>
        </div>
    </div>
    
    <div class="mb-2">
        <label class="form-label fw-bold text-danger border-bottom border-danger pb-1"><i class="fas fa-box-open me-1"></i> Daftar Dokumen / Barang Hilang</label>
        <div id="dokumen-container" class="mt-2">
            <div class="dokumen-item shadow-sm bg-white p-2 rounded">
                <div class="input-group">
                    <span class="input-group-text bg-light text-muted">1</span>
                    <input type="text" class="form-control" name="dokumen[]" placeholder="Contoh: KTP atas nama fulan / SIM A / BPKB Mobil..." value="SIM A">
                </div>
                <button type="button" class="btn-remove-dokumen shadow-sm" onclick="removeDokumen(this)" style="display: none;" title="Hapus Baris">
                    <i class="fas fa-times"></i>
                </button>
            </div>
        </div>
        <button type="button" class="btn-add-dokumen mt-3 shadow-sm" onclick="tambahDokumen()">
        ...
        <div class="editor-workspace">
            <div class="editor-container paper-a4" id="editor" contenteditable="true" style="padding: 2cm 2cm 2cm 3cm; font-family: 'Times New Roman', Times, serif; font-size: 12pt;">
                ...
                <div style="text-align: center; margin: 15px 0 20px; color: #000;">
                    <h4 style="text-decoration: underline; font-weight: bold; margin:0 0 3px 0; font-size: 13pt; color: #000;">SURAT KETERANGAN KEHILANGAN</h4>
                ...
                <div style="color: #000;">
                    ...
                    <p style="margin-bottom: 8px;">menerangkan dengan sebenarnya bahwa orang tersebut diatas pada hari <span id="hari_text" contenteditable="true" style="font-weight:bold;"><?php echo hari_indonesia(date('Y-m-d')); ?></span> tanggal <span id="tanggal_text" contenteditable="true" style="font-weight:bold;"><?php echo date('d-m-Y'); ?></span> sekitar pukul <span id="jam_text" contenteditable="true" style="font-weight:bold;"><?php echo date('H:i'); ?></span> telah kehilangan :</p>
                    
                    <div id="dokumen_list" style="margin-bottom: 8px; font-weight: bold;">
                        <p style="margin: 3px 0 3px 20px;">1. SIM A</p>
                    </div>
                    
                    <p style="margin-bottom: 10px;">di <span id="lokasi_text" contenteditable="true" style="font-weight:bold;">Diantara Jalan Sukolilo Sampai Jalan Pasar Labang</span>.</p>
                ...
                <div class="col-lg-5 mb-4 mb-lg-0 border-end pe-4">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control" id="keperluan" name="keperluan" placeholder="Misal: Lapor Polisi">
                        </div>
                    </div>
                </div>
                    ...
                    <table class="action-table">
                        ...
                        <tr>
                            <td colspan="2" style="padding-top: 5px;">
                                <a href="kehilangan.php" class="btn-action btn-preview" style="background: #fdfdfd;">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0">
            ...
            <div class="modal-body text-center py-5">
                ...
                <p class="text-muted mb-4">Pastikan data kehilangan dan data pemohon sudah benar sebelum menyimpan.</p>
                <div class="alert alert-light border text-start shadow-sm mx-auto" style="max-width: 350px;">
                    ...
                    <div><strong>Pemohon:</strong> <br><span class="text-dark font-weight-bold text-uppercase"><?php echo $penduduk['nama_penduduk'] ?? ''; ?></span></div>
                </div>
            </div>
...
// Helper untuk Auto-Capitalize agar kursor tidak lompat
...
// ===== FUNGSI UNTUK SINKRONISASI FORM KEJADIAN KE EDITOR =====
function initKejadianListeners() {
    ...
    if (jamInput) jamInput.addEventListener('input', updateEditorJam);
    if (lokasiInput) lokasiInput.addEventListener('input', updateLokasi);
...
function updateHariKejadian() {
    ...
    if (hariText) hariText.textContent = hariIndonesia;
}

function updateEditorTanggal() {
    const tanggal = document.getElementById('tanggal_kejadian').value;
    if (!tanggal) return;
    const tgl = new Date(tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' }).split('/').join('-');
    const tanggalText = document.getElementById('tanggal_text');
    if (tanggalText) tanggalText.textContent = tgl;
}
...
function updateDokumenList() {
    ...
    dokumenInputs.forEach((input, index) => {
        if (input.value.trim()) {
            html += `<p style="margin: 3px 0 3px 20px;">${index + 1}. ${input.value}</p>`;
        }
    });
    
    if (dokumenList) dokumenList.innerHTML = html || '<p style="margin: 3px 0 3px 20px;">-</p>';
}
...
function tambahDokumen() {
    ...
    newItem.className = 'dokumen-item shadow-sm bg-white p-2 rounded';
    newItem.innerHTML = `
        <div class="input-group">
            <span class="input-group-text bg-light text-muted">${index}</span>
            <input type="text" class="form-control" name="dokumen[]" placeholder="Masukan nama barang hilang...">
        </div>
        <button type="button" class="btn-remove-dokumen shadow-sm ms-2" onclick="removeDokumen(this)" title="Hapus Baris">
            <i class="fas fa-times"></i>
        </button>
    `;
...
function removeDokumen(btn) {
    ...
    if (container.children.length > 1) {
        ...
        // Perbarui nomor urut di UI input group
        Array.from(container.children).forEach((item, idx) => {
            const spanNumber = item.querySelector('.input-group-text');
            if (spanNumber) spanNumber.textContent = idx + 1;
        });
        ...
        updateDokumenList(); // Update kertas editor
    }
}

function updateHiddenInputs() {
    document.getElementById('hidden_tanggal_kejadian').value = document.getElementById('tanggal_kejadian').value;
    document.getElementById('hidden_jam_kejadian').value = document.getElementById('jam_kejadian').value;
    document.getElementById('hidden_hari_kejadian').value = document.getElementById('hari_kejadian').value;
    document.getElementById('hidden_lokasi_kejadian').value = document.getElementById('lokasi_kejadian').value;
    
    const dokumenInputs = document.querySelectorAll('input[name="dokumen[]"]');
    let barangList = [];
    dokumenInputs.forEach(input => {
        if (input.value.trim()) barangList.push(input.value.trim());
    });
    document.getElementById('hidden_barang_hilang').value = barangList.join('\n');
}
            ...
            fetch('kehilangan.php?search_penduduk=' + encodeURIComponent(keyword))
                ...
                .then(data => {
                    if (data.length > 0) {
                        ...
                        searchResults.innerHTML = '<div class="search-result-item text-muted text-center" style="justify-content: center;"><i class="fas fa-info-circle me-1"></i> Tidak ada data ditemukan</div>';
                        searchResults.style.display = 'block';
                    }
                });
        }, 300);
...
function submitFormWithAction(action, blank = false) {
    updateHiddenInputs(); // Wajib untuk form kehilangan
